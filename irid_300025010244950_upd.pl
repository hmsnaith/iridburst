...
$mooring="PAP201604";   # mred May 2010   changed for 2014
...
#if ($data =~ m/660d0028449ce246,1,DAS 2/)  changed jul2014
#if ($data =~ m/5206001e4525d187,1,DAS 1/)
if ($data =~ m/,1,DAS/) # 20160414  more generalised so may not need changing in future
    {
    ($WMAN,$data_date,$data_time,$WMAN_sn,$mess_no,$sys_id,$lat0,$lon0,$wind_speed,$wind_dir,$wind_gust,    $sea_temp, $air_temp, $humidity, $dew_temp,  $air_press, $Hsig, $Hmax, $wave_TP, $wave_dir, $last)=split(/,/,$data,21);
    ...
    if (substr($lon0,4,1) eq ".")
		{$lon1=(substr($lon0,0,2) + (substr($lon0,2,7)/60))*-1;}
		else
		{$lon1=(substr($lon0,0,1) + (substr($lon0,1,7)/60))*-1;}  #for when longitude is less than 10 degrees
...
#			|| die "Update $sql failed";
			...
			$sqlupd.="wind_gust='".$wind_gust."',";					
			$sqlupd.="wave_TP='".$wave_TP."',";
			$sqlupd.="air_temp='".$air_temp."',";
			$sqlupd.="dew_temp='".$dew_temp."',";
			$sqlupd.="humidity='".$humidity."',";
#			$sqlupd.="air_temp_2='".$air_temp_2."',";
#			$sqlupd.="dew_temp_2='".$dew_temp_2."',";
#			$sqlupd.="humidity_2='".$humidity_2."',";
			$sqlupd.="air_press='".$air_press."',";
			$sqlupd.="Hsig='".$Hsig."',";
			$sqlupd.="Hmax='".$Hmax."',";
			$sqlupd.="wave_dir='".$wave_dir."',";
			$sqlupd.="wave_spread='".$wave_spread."',";
			$sqlupd.="Lat='".$lat1."',";
			$sqlupd.="Lon='".$lon1."',";
...
#			print ("METsql $sqlupd \n");
			...
			printf DATAOUT "$WMAN,$data_date,$data_time,$WMAN_sn,$mess_no,$lat0,$lon0,$wind_speed,$wind_dir,$wind_gust,
