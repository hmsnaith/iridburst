...
exit;
# code only neded to fit in with PAP pattern
...
$mooring="CIS201206";   # mred June 2012
...
# leave insueful to have 2 running modes
$nightly=0;
if ($nightly>0)  
	{$file_dir0.="concat/";
...
#not working when message number went over 999 mrp 20111108
    ($rest,$data_type0)=split(/-/,$data,2);
    $data_type=substr($data_type0,0,2);
#    $data_date=substr($data,9,23);
    $data_date=substr($data_type0,3,23);
  print("DATA  xxrecord $data_type      ::$data \n");
##########################################################
    if (($data_start eq "!{") and ($data_type eq "60"))
       {
       print("DATA_TYPE60: $data_type\n");
# aquadrop
       #read until Sampledata or next 513 record
       $data=<STDIN>;
       unless (($data =~ m/Remo/ ) or ($data =~ m/SampleData/))
        {$data=<STDIN>;}
 #       print("DATA READ in 60:$data\n");
	 if ($data =~ m/SampleData/)
	 {
	    $tagblock="SampleData";
	    $bgcsample = &cutTagBlock($data,$tagblock);
	    print "SAMPLEDATA 60 $bgcsample \n";
	    if ($bgcsample =~ m/>/)
	       {($rest,$data)=split(/>/,$bgcsample,2);
	         $bgcsample=$data;
	       }
...
#	       print("SAMPLEDATA1 $bgc_type:$id:\n");
	       ($x, $aq_mon,$aq_day,$aq_year,$aq_h,$aq_m,$aq_s,$code1,$code2,$v1,$v2,$v3,$a1,$a2,$a3,$aq_batt,$aq_SoS,$heading,$pitch,$roll,$aq_press1000,$aq_temp1000,$code3,$code4,$aq_speed,$aq_dir)=split(/\s/,$bgcsample);
               print "AQUADROP data $aq_mon,$aq_day,$aq_year,$aq_h,$aq_m,$aq_s,$code1,$code2,$v1,$v2,$v3,$a1,$a2,$a3,$aq_batt,$aq_SoS,$heading,$pitch,$roll,$aq_press1000,$aq_temp1000,$code3,$code4,$aq_speed,$aq_dir\n";
		
	       $rec_date_time=sprintf("%04d-%02d-%02d %02d:%02d:%02d",$aq_year,$aq_mon,$aq_day,$aq_h,$aq_m,$aq_s);   
	
	print "BGC60 $rec_date_time \n";
	
		$sqlsel="SELECT *  FROM ".$mooring."_aquadrop WHERE Date_Time = '".$rec_date_time."'";
		print("$sqlsel\n");
		$sel = $dbh->prepare($sqlsel);    
		$sel->execute(); 
#		|| die "Update $sql failed";
		...
		unless (@rowh)
			{      
			$sql="INSERT INTO ".$mooring."_aquadrop SET Date_Time='".$rec_date_time."'";
			$ins = $dbh->prepare($sql);    
			$ins->execute()  || die "insert $sql failed";
							}	
	
		        
			$sqlupd="UPDATE ".$mooring."_aquadrop SET  code1='".$code1."',";
			$sqlupd.="code2='".$code2."',";
			$sqlupd.="v1='".$v1."',";
			$sqlupd.="v2='".$v2."',";
			$sqlupd.="v3='".$v3."',";
			...
			$sqlupd.="a2='".$a2."',";
			$sqlupd.="a3='".$a3."',";
			$sqlupd.="battery='".$aq_batt."',";
			$sqlupd.="SoS='".$aq_SoS."',";
			$sqlupd.="heading='".$heading."',";
			$sqlupd.="pitch='".$pitch."',";
			$sqlupd.="roll='".$roll."',";
			$sqlupd.="press1000='".$aq_press1000."',";
			$sqlupd.="temp1000='".$aq_temp1000."',";
			$sqlupd.="code3='".$code3."',";
			$sqlupd.="code4='".$code4."',";
			$sqlupd.="speed='".$aq_speed."',";
			$sqlupd.="dir='".$aq_dir."',";
			$sqlupd.=" add_dat='".$nowdate."' ";
			$sqlupd.=" where Date_Time = '".$rec_date_time."'";
		print ("AQUADROP sql $sqlupd \n");
			$ins = $dbh->prepare($sqlupd);    
			$ins->execute()|| die "Update $sql failed";
		...
		#{ # invalid sn  bgc type combination
		    print ("END of Aquadrop serial number $id \n");           
	        #} 
         } # end of writing aquadrop current meter record
         } # end of datatype 60
  ########################################################       
...
#	       print("SAMPLEDATA1 $bgc_type:$id:\n");
	       ...
	       $rec_date_time=substr($data_date,0,13).":00:00";   # force to be on hour
...
#		print ("BGC sql $sqlupd \n");
  ...
  ########################################################       
	...
	# potential microcat data
        if (($data_start eq "!{") and  ($data_type<20) )   
	{
	 print("DATA_TYPE $data_type\n");

	print "potential microcat record $data \n";
         $data=<STDIN>;
	($mc_sn,$mc_cond,$mc_temp,$mc_rest)=split(/,/,$data,4);
#	print("MICROCAT0 $data:$mc_sn:$data_type:$sn[$data_type]\n");
        if (substr($mc_sn,0,1) ne " ")    # mrp 120612 fixing rogue characters at start of string
         { substr($mc_sn,0,1) = " ";}
...
#	   print "MICROCAT $data \n";
	      ...
	      $rec_date_time=substr($data_date,0,13).":00:00";   # force to be quarter past !!!! may be problem for microcats so will need chacking
